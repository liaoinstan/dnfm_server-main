...
#   方案二是单独为有怪的情况计时，不管有没有怪，1分钟内没过图就认为被卡位（狮子头房2分钟）
#   
# 2.布万加上路：虽然目前的脚本不支持上路，如果进入了上路房间角色会自行返回，但是如果在上路连续进错两个房间，就无法识别当前位置了
#   导致人物失去目标，考虑更新房间算法，把上路房间也包括进去
#
# 3.选择角色页面，目前只能支持选择第一排的英雄，有待优化
# 4.广告页面，切换角色时经常弹出广告，需要对广告做识别并关闭
# 5.模版匹配优化，目前匹配是全屏匹配，性能有浪费，需要进行优化做局部匹配
# 6.疲劳检查，在没有引入文字库之前，目前没有比较好的方案
# 7.300疲劳弹窗，需要识别并关闭
##########################################################################################################################